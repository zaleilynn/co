...
void StateHandler(Executor* ptr, ContainerState state) {
    ...
    if (state == CONTAINER_STARTED) {
        ...
        Proxy<SchedulerClient> proxy = Rpc<SchedulerClient, SchedulerClient>::GetProxy(FLAGS_scheduler_endpoint, TIME_OUT);
        try {
...
void* ResourceInfoReceiver(void* unused) {
    while (true) {
        ...
        try {
            ...
            Proxy<CollectorClient> proxy = Rpc<CollectorClient, CollectorClient>::GetProxy(FLAGS_collector_endpoint, TIME_OUT);
            proxy().Heartbeat(info.Get());
            DLOG(WARNING) << "Send heartbeat success";
        } catch (TException &tx) {
...
void* ExecutorStatusReceiver(void* unused) {
    while (true) {
        ...
        // get executor id
        vector<string> info;
        DLOG(WARNING) << "debug: receive state msg: " << msg.Get();
        ...
        //FindToDo去container pool里面查找了指定id的指针
