...
void Container::SetName() {
    char data[50] = {0};
    snprintf(data, sizeof(data), "%s_%ld", m_info.framework_name.c_str(),
             m_info.id);
    m_name = data;
}

int Container::Init() {
    DLOG(WARNING) << "debug: in container init";
    SetName();
    ...
    if (access(framework_dir.c_str(), F_OK) < 0) {
        ...
        DLOG(WARNING) << "debug: create framework directory: " << framework_dir;
    }
    ...
    DLOG(WARNING) << "debug: create work directory: " << path;
...
void Container::RedirectLog() {
    ...
    char log_name[100] = {0};
    snprintf(log_name, sizeof(log_name), "%s_%ld_%s",
...
int Container::Execute() {
    ...
    for (int i = 0; m_c_args[i]; ++i)
        oss << m_c_args[i] << " ";
    DLOG(WARNING) << "debug: command argument list: " << oss.str();
    DLOG(WARNING) << "debug: start Executor ID: " << m_info.id 
                  << " in container: " << m_name;
    m_pid = fork();
    int ret = 0;
    // child process
    // 用于启动container 
    if (m_pid == 0) {
        // 虽然是子进程，但是这个代码块过后就exit了
        // close all the fd inherited from parent
        // 要不在这里报container started?
        // 这个里面用glog容易出事情,会创建一个新的log文件
        CloseInheritedFD();
        RedirectLog();
        ...
        //等它反应一下？
        //上面的运行如果失败了这里也会报container started,这个是错误的消息
        sleep(1);
        ContainerStarted();
        return ret;
    }
...
void Container::ContainerFinished() {
    DLOG(WARNING) << "debug: dealing container Finished message: ID:" << m_info.id << " PID:" << m_pid;
    // change status
...
void Container::ContainerStarted() {
    DLOG(WARNING) << "debug: dealing container Started messsage: ID: " << m_info.id << " PID:" << m_pid;
    // set resource limit
    SetResourceLimit();
    // change status
    WriteLocker locker(m_lock);
...
int Container::GetMemory() {
    ...
    DLOG(WARNING) << "debug: USED MEMORY:" << value;
    int memory;
    ...
    if (value > 0)
        ...
        memory = 0;
    DLOG(WARNING) << "debug: framework:" << m_info.framework_name
...
double Container::GetCpuUsage(double used_cpu) {
    ...
    if (cur_cpu > 0) {
        if (m_first) {
            ...
            DLOG(WARNING) << "debug: CURRENT CPU : " << m_prev_cpu
        ...
        DLOG(WARNING) << "debug: true cpu usage :" << cpu_usage;
        cpu_usage /= m_info.need_cpu / used_cpu;
        DLOG(WARNING) << "debug: corrected cpu usage:" << cpu_usage;
        m_prev_cpu = cur_cpu;
...
int Container::GetChildrenNum() {
    ...
    free(res);
    DLOG(WARNING) << "debug: child process number: " << vt.size() - 2;
