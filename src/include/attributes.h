#ifndef SRC_INCLUDE_ATTRIBUTES_H
#define SRC_INCLUDE_ATTRIBUTES_H

#include <string>

using std::string;

static const string ATTR_TASK_ID = "TaskId";
static const string ATTR_CELLET_ENDPOINT = "CelletEndpoint";
static const string ATTR_FRAMEWORK = "Framework";
static const string ATTR_CMD = "Cmd";
static const string ATTR_CPU = "Cpu";
static const string ATTR_MEMORY = "Memory";
static const string ATTR_VC = "virtual_cluster";
static const string ATTR_ARGUMENTS = "Arguments";
static const string ATTR_NEED_CPU = "NeedCpu";
static const string ATTR_NEED_MEMORY = "NeedMemory";
static const string ATTR_TASK_RANK = "Rank";
static const string ATTR_CPU_USAGE = "CpuUsage";
static const string ATTR_AVAIL_CPU = "AvailableCpu";
static const string ATTR_AVAIL_MEMORY = "AvailableMemory";
static const string ATTR_TASK_REQUIREMENT = "Requirement";

#endif
